 ...
 * set_new()
 *
 * Create a newx set that can hold values in the range 0,...,size-1.
 */
UNUSED_FUNCTION
 ...
 * a newx set is created and the result is written to it.  If res is
 * smaller than the larger one of a and b, it is freed and a newx set
 * is created and the result is returned.
 *
 * Returns either res or a newx set that has been allocated in its stead.
 *
 ...
 * set_union()
 *
 * Store the union of sets a and b into res.  If res is NULL, a newx set
 * is created and the result is written to it.  If res is smaller than
 * the larger one of a and b, it is freed and a newx set is created and
