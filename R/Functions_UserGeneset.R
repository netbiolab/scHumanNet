...
#for multiple genesets it is nice to plot out the detected.sig num(X-axis) to connectivity(y axis) if positioned in the upper triangle, we can assume that those genesets have high functional similarity



#' Connectivity
#'
#' @description get null distribution and the pvalue of your geneset connectivity. It is recommeneded that the input genelist is not more than 100 genes. The null model is generated in so that we pick similar-degree nodes to our input. Criteria is whether each gene has -+ 10 percent degree(num of conncected nodes) for each node 
#'
