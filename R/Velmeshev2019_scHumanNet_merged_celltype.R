...
#for condition and celltype, rearrange columns
colnames(test) <- names(rank.df.final)
test <- test[,c("Control_Excitatory","ASD_Excitatory","Control_Inhibitory","ASD_Inhibitory",
                "Control_Microglia","ASD_Microglia","Control_OPC" ,"ASD_OPC",
                "Control_Astrocyte","ASD_Astrocyte","Control_Oligodendrocytes","ASD_Oligodendrocytes",
                "Control_Endothelial","ASD_Endothelial","Control_Others","ASD_Others")]
write.table(test, 'top50_genes_Brain_condition_celltype_MTexcluded.tsv', quote = F, col.names = T, row.names = F, sep = '\t')


rank.df.final <- rank.df.final[,c("Control_Excitatory","ASD_Excitatory","Control_Inhibitory","ASD_Inhibitory",
                "Control_Microglia","ASD_Microglia","Control_OPC" ,"ASD_OPC",
                "Control_Astrocyte","ASD_Astrocyte","Control_Oligodendrocytes","ASD_Oligodendrocytes",
                "Control_Endothelial","ASD_Endothelial","Control_Others","ASD_Others")]

brain.gene <- cbind(rownames(rank.df.final), rank.df.final)
write.table(brain.gene, quote = F, col.names = T, row.names = F, sep = '\t',file = './Brain_LLS_strength_rank.tsv')
...
for (celltype in names(table(meta$celltypes_merged))){
  ...
  df <- rank.df.final[,c(paste0('Control_', celltype), paste0('ASD_', celltype))]
